...
INVALID_DEPARTURE = {
    ...
    "mon ami", "quelqu'un",
    # Pronoms/adverbes de lieu non-spécifiques
    "ici", "là", "là-bas", "d'ici", "quelque part",
}

# Patterns qui ne devraient PAS être Date
INVALID_DATE_PATTERNS = [
    # Villes béninoises courantes (ne sont pas des dates)
...
def post_process(entities_dict):
    ...
    for etype, mentions in entities_dict.items():
        ...
        for mention, score in mentions:
            ...
            # Date: filtrer les noms de villes mal classés comme Date
            if etype == "Date":
                skip = False
                for city in INVALID_DATE_PATTERNS:
                    if city in m_lower:
                        skip = True
                        break
                if skip:
